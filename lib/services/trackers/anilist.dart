...
class AniListTracker extends Tracker {
  ...
  Future<OAuthCredentials> login() async {
    ...
    final fragment = Uri.splitQueryString(
      Uri.parse(resultUrl.replaceAll('#', '?')).query,
    );
    ...
    protectedCredentials = creds;
    await persistCredentials(creds, cipher: protectedCipher);
    return creds;
  }
    ...
    return media.cast<Map<String, dynamic>>().map((m) {
      ...
      return TrackerSearchResult(
        ...
        score: (m['meanScore'] as num?)?.toDouble() != null
            ...
            : null,
        startDate: _formatStartDate(m['startDate'] as Map<String, dynamic>?),
  ...
  void _mergeRemoteEntry(Track track, Map<String, dynamic> entry) {
    ...
    track.startReadAt =
        _fuzzyDateToMillis(entry['startedAt'] as Map<String, dynamic>?);
    track.finishReadAt =
        _fuzzyDateToMillis(entry['completedAt'] as Map<String, dynamic>?);
