...
class AnimePlayerSettingsNotifier extends StateNotifier<AnimePlayerSettings> {
  ...
  void setBrightness(double b) => state = state.copyWith(defaultBrightness: b);
...
final animePlayerSettingsProvider =
    StateNotifierProvider<AnimePlayerSettingsNotifier, AnimePlayerSettings>(
...
class _AnimePlayerViewState extends ConsumerState<AnimePlayerView>
  ...
  Future<void> _initBrightness() async {
    try {
      final initial = await _brightness.application;
  ...
  Future<void> _openVideo() async {
    ...
    final qualities =
        hosters.isNotEmpty ? hosters[_hosterIndex].qualities : <VideoQuality>[];
  ...
  Future<void> _setBrightness(double v) async {
    ...
    try {
      await _brightness.setApplicationScreenBrightness(v);
    } catch (_) {
  ...
  Future<void> _setAudio(int index) async {
    ...
    try {
      ...
      await _player.setAudioTrack(
        AudioTrack(tracks[index].id, tracks[index].title, tracks[index].lang),
      );
    } on Object {
  ...
  Future<void> _screenshot() async {
    try {
      ...
      if (bytes == null) {
        if (mounted) showSnack(ref, context, 'Screenshot unavailable');
        return;
      }
      ...
      if (mounted) showSnack(ref, context, 'Screenshot saved');
    } catch (_) {
      if (mounted) showSnack(ref, context, 'Screenshot failed');
    }
  }
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      onTap: () async {
                        ...
                        final hosters = ref.read(hosterTreeProvider(ep.id));
                        final url =
...
class PlayerGradientBottom extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bufMs =
        buffered.inMilliseconds.toDouble().clamp(0, totalMs).toDouble();
    return Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(
              ...
              Wrap(
                ...
                children: [
                  ...
                  ActionChip(
                    ...
                    avatar: const Icon(Icons.fast_forward, color: Colors.white),
                    onPressed: onAniSkip,
                  ),
                  ActionChip(
