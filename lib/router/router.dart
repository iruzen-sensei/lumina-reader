...
/// The app router. Plain Riverpod provider (converted from codegen —
/// riverpod_generator was dropped: no compatible version exists alongside
/// isar_generator 3.x).
final routerProvider = Provider<GoRouter>((ref) {
  return GoRouter(
    ...
    routes: [
      ...
      // Manga reader — resolves the parent manga from the chapter id so
      // deep links from history / updates work without an extra param.
      GoRoute(
      ...
      GoRoute(
        ...
        builder: (context, state) => NovelReaderScreen(
          id: int.parse(state.pathParameters['chapterId']!),
        ),
      ),
      // EPUB reader — loads the file path from the library entry
      GoRoute(
      ...
      // PDF reader — loads the file path from the library entry
      GoRoute(
      ...
      // Anime player — resolves the parent anime from the episode id
      GoRoute(
        path: '/animePlayer/:episodeId',
        name: 'animePlayer',
        builder: (context, state) => AnimePlayerScreen(
