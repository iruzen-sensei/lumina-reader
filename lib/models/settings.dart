...
class Settings {
  ...
  /// E-ink optimized mode: high contrast, no animations, white background.
  bool? einkMode;

  /// App-wide base font size (sp).
  int? uiFontSize;
  ...
  /// Whether to require app lock immediately at launch.
  bool? lockOnLaunch;
  ...
  /// Timestamp (ms since epoch) of the last successful backup.
  int? backupLastBackupAt;
  ...
  // NOTE: the previous `mergeWith` implementation was a stub that returned
  // `Settings(id: id)` — silently wiping every field. It has been REMOVED.
  // Merge semantics now live in `models/mappers.dart::mergeSettings`, which
  // merges the fields the app actually persists, explicitly, field by field.
  // If you add a persisted setting, add it to BOTH `mappers.dart` and the
  // relevant repository method — never reintroduce a blind merge.
